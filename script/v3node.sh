...
update_shell() {
    ...
    if ! download_verified_script_asset "$release_repo" "$manager_ref" v3node.sh "$temporary" \
        && ! download_verified_script_asset "$release_repo" "$manager_ref" znode.sh "$temporary"; then
        rm -f "$temporary"
        echo ""
